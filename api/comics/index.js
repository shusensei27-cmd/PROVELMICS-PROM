...
module.exports = async function handler(req, res) {
  ...
  // ── POST submit komik baru (dengan validasi & perbaikan URL) ──
  if (req.method === 'POST') {
    const user = requireAuth(req, res);
    if (!user) return;

    try {
      let { title, synopsis, genre, image_urls, cover_url } = req.body;
      ...
      // Bersihkan dan validasi image_urls
      let cleanImageUrls = [];
      if (Array.isArray(image_urls)) {
        cleanImageUrls = image_urls
          .map(url => (typeof url === 'string' ? url.trim() : ''))
          .filter(url => url.length > 0 && (url.startsWith('http://') || url.startsWith('https://')));
      } else if (typeof image_urls === 'string') {
        // Jika frontend mengirim sebagai string (satu baris atau multiline)
        cleanImageUrls = image_urls
          .split('\n')
          .map(line => line.trim())
      ...
      if (cleanImageUrls.length === 0) {
        return res.status(400).json({ error: 'At least one valid image URL is required (must start with http:// or https://)' });
      }

      // Siapkan data
      const id = uuidv4();
      const genreStr = JSON.stringify(Array.isArray(genre) ? genre : (genre ? [genre] : []));
      const imagesStr = JSON.stringify(cleanImageUrls);
      const userId = user.sub || user.id;
      ...
      console.log(`[comics] Komik berhasil dibuat: ${id} oleh ${userId}`);
      return res.status(201).json({ message: 'Comic submitted for review', id, status: 'pending' });

    } catch (err) {
      console.error('Create comic error:', err);
      // Kirim error detail ke frontend (untuk debugging, nanti bisa dihapus)
      return res.status(500).json({ error: 'Failed to submit comic', detail: err.message });
    }
  }
